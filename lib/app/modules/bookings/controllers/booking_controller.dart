...
class BookingController extends GetxController {
  ...
  Future refreshBooking({bool showMessage = false}) async {
    await getBooking();
    if (showMessage) {
      Get.showSnackbar(Ui.SuccessSnackBar(message: "booking_page_refreshed_successfully".tr));
    }
  }
  ...
  Future<void> getBooking() async {
    try {
      ...
      if (booking.value.status == Get.find<BookingsController>().getStatusByOrder(Get.find<GlobalService>().global.value.accepted) && timer == null) {
...
/*  Future<void> onTheWayBookingService() async {
  ...
  Future<void> startBookingService() async {
    try {
      final _status = Get.find<BookingsController>().getStatusByOrder(Get.find<GlobalService>().global.value.accepted);
  ...
  Future<void> cancelBookingService() async {
    try {
      if (booking.value.status.order < Get.find<GlobalService>().global.value.accepted) {
        final _status = Get.find<BookingsController>().getStatusByOrder(Get.find<GlobalService>().global.value.failed);
        final _booking = new Booking(id: booking.value.id, cancel: true, changeStatus: true,status: _status);
        await _bookingRepository.update(_booking);
        booking.update((val) {
          val.cancel = true;
          val.status = _status;
  ...
  void onRescheduleBookingService(){
    Get.toNamed(Routes.BOOK_E_SERVICE, arguments: {'booking': booking.value});
  }
}
