...
import 'package:beauty_salons_customer/app/modules/salon/controllers/salon_e_services_controller.dart';
...
class SalonController extends GetxController {
  ...
  SalonEServicesController eServicesController;
  ...
  void onInit() {
    ...
    scrollController.addListener(() async {
      if (scrollController.position.pixels ==
        ...
        if (Get.isRegistered<SalonEServicesController>()) {
          eServicesController = Get.find<SalonEServicesController>();
        } else {
          eServicesController = Get.put(SalonEServicesController());
        }
        if (currentIndex == 1 && !eServicesController.isDone.value) {
          if (eServicesController.page > 10) {
            Get.bottomSheet(
              EServicesFilterBottomSheetWidget(),
              isScrollControlled: true,
            );
            Get.showSnackbar(Ui.SuccessSnackBar(
                message: "specify_what_exactly_you_are_looking_for".tr));
          } else {
            await eServicesController.loadEServicesOfCategory();
          }
        }
      }
    });
  ...
  Future refreshSalon({bool showMessage = false}) async {
    ...
    if (showMessage) {
      Get.showSnackbar(Ui.SuccessSnackBar(
          message: salon.value.name + " " + "page_refreshed_successfully".tr));
    }
  }
