...
import '../modules/rating/controllers/rating_controller.dart';
import '../modules/rating/views/rating_dialog.dart';
...
class FireBaseMessagingService extends GetxService {
  ...
  void _bookingNotification(RemoteMessage message) async {
    print("Remote Data: ${message.data}");
    if (message.data["status"] == Get.find<GlobalService>().global.value.done.toString()) {
      BookingRepository _bookingRepository = BookingRepository();
      var booking = await _bookingRepository.get(message.data["bookingId"]);
      RatingController ratingController;
      if (Get.isRegistered<RatingController>()) {
        ratingController = Get.find();
        ratingController.review.value.booking = booking;
      } else {
