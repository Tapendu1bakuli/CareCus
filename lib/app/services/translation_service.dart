  ...
  Future<TranslationService> init() async {
    ...
    if(box.read("language_selection") == 1){
      ...
      if(languages.contains(defaultLocale.toString().split("_").first)){
        ...
        Get.find<SettingsService>().setting.value.mobileLanguage = defaultLocale.toString().split("_").first;
        await loadTranslation(locale: defaultLocale.toString().split("_").first);
      }else{
        box.write('language_selection', 1);
        box.write('language', Get.find<SettingsService>().setting.value.mobileLanguage);
        await loadTranslation(locale: Get.find<SettingsService>().setting.value.mobileLanguage);
      }
    }
